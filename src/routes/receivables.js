...
receivableRoutes.get('/collections', requirePermission('RECEIVABLES','VIEW'), async c => {
  ...
  const rows = await all(c.env.DB, `SELECT * FROM erp_ar_collections ${w}
    ORDER BY txn_date DESC, id DESC LIMIT ? OFFSET ?`, [...args, size, offset]);
...
receivableRoutes.get('/summary', requirePermission('RECEIVABLES','VIEW'), async c => {
  ...
  const [totals, byStream, byMonth, byCustomer] = await Promise.all([
