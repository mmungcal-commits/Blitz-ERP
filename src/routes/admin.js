...
adminRoutes.get('/access-audit', requirePermission('ADMIN','MANAGE'), async c=>{
const q=`%${normalizeText(c.req.query('q'))}%`;
const rows=await all(c.env.DB,`SELECT id,event_at,user_email,environment,action,module,record_type,
record_id,record_no,request_id,ip_address
FROM erp_audit_log
WHERE ?='%%' OR user_email LIKE ? OR action LIKE ? OR module LIKE ? OR record_no LIKE ?
ORDER BY event_at DESC,id DESC LIMIT 1000`,[q,q,q,q,q]);
...
departments:{table:'erp_departments',label:'Departments',list:['code','name'],edit:['name','code'],search:['name','code'],active:true,order:'name',create:true}
};
...
const entities=Object.keys(RECORD_ENTITIES).map(k=>({key:k,label:RECORD_ENTITIES[k].label,columns:RECORD_ENTITIES[k].list,editable:RECORD_ENTITIES[k].edit,create:!!RECORD_ENTITIES[k].create}));
...
adminRoutes.post('/records/:entity/new', requireUser, async c=>{
const def=RECORD_ENTITIES[c.req.param('entity')]; if(!def)return fail(c,'Unknown record type',404);
if(!def.create)return fail(c,'This record type cannot be created here.');
const b=await jsonBody(c);
if(!await recordGate(c,b.passcode))return fail(c,'A valid edit passcode is required to add records.',403);
const data=b.data||b.changes||{};const cols=[];const marks=[];const args=[];
for(const col of def.edit){ if(Object.prototype.hasOwnProperty.call(data,col)&&data[col]!==''&&data[col]!=null){ cols.push(col); marks.push('?'); args.push(data[col]); } }
if(!cols.length)return fail(c,'Provide at least one field to create the record.');
let r;
try{ r=await run(c.env.DB,`INSERT INTO ${def.table}(${cols.join(',')}) VALUES(${marks.join(',')})`,args); }
catch(err){ return fail(c,/UNIQUE/i.test(err.message||'')?'A record with those details already exists.':(err.message||'Create failed.')); }
await audit(c,{action:'RECORD_CREATE',module:'ADMIN',recordType:def.table,recordId:r.meta.last_row_id,after:data});
return ok(c,{created:true,id:r.meta.last_row_id},201);
});
