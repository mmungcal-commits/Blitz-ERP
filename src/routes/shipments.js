...
shipmentRoutes.get('/:id', requirePermission('SHIPMENTS','VIEW'), async(c)=>{
  ...
  const [lines,assets,receipts]=await Promise.all([
    ...
    all(c.env.DB,`SELECT a.*,i.item_name FROM erp_expected_assets a LEFT JOIN erp_items i ON i.id=a.item_id WHERE shipment_id=? ORDER BY source_sheet,source_row LIMIT 5000`,[id]),
...
shipmentRoutes.post('/:id/status', requirePermission('SHIPMENTS','APPROVE'), async(c)=>{
  const id=Number(c.req.param('id')); const b=await jsonBody(c); const allowed=['DRAFT','MANIFESTED','IN_TRANSIT','ARRIVED','RECEIVING','PARTIALLY_RECEIVED','RECEIVED','CLOSED','CANCELLED'];
