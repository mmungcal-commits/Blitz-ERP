...
def main():
    ...
    test('Actual source workbooks embedded',scalar(db,'select count(*) from erp_opening_data_control')>=0,scalar(db,'select count(*) from erp_opening_data_control'))
    test('Source rows archived',scalar(db,'select count(*) from erp_import_rows')>=0,scalar(db,'select count(*) from erp_import_rows'))
